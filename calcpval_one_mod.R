...
x <- rowSums(rawcount==0)!=ncol(rawcount)
newCountTable <- rawcount[x,]
...
counts <- newCountTable[,labelAllRow]
...
gene <- as.character(newCountTable[sampleList,"gene"])
ctrlFstCounts <- rowSums(newCountTable[sampleList, OCcolumns])
ctrlSndCounts <- rowSums(newCountTable[sampleList, ACcolumns])
objFstCounts <- rowSums(newCountTable[sampleList, ODcolumns])
objSndCounts <- rowSums(newCountTable[sampleList, ADcolumns])
